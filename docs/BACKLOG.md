...
> end-to-end**, above doc 40's projection *and* its perfect-overlap floor), correctness veto passed
> on all four gates, and it is now the default (`config.stitch_backend = "tifffile"`). Phase D drops
> from 32.3% to **20.3% of the `workers=4` wall**. Peak RSS also fell **45.6 → 17.0 GB**, which
> relaxes doc 27 §6's 61–62 GB host requirement. Candidate C is recommended **closed**, not
> deferred. Still owed from that round: the full-slide **error-bar campaign is untouched** — every
> full-slide comparison in this project remains n=1 vs n=1 — and `overlay_pyramid_audit.py` still
> tests brightness only, so it would pass a file with a tile-placement defect.
> Previously updated 2026-07-29 for round 11
...
> 4.21% of wall and prefetch already hides 99.8% of it. Item 1b (Phase D stitch) was **CLOSED
> NEGATIVE** at that point — round 10 re-measured the GPU/CPU-container port with the real read/join
> cost included and both candidates came back *slower* than the shipped baseline — **superseded:
> round 12 found the one lever doc 35 left unbuilt and round 13 shipped it (see above)**. Item 7b
> (allocator OOM) is **root-caused and mitigated** — round 11 swept
...
| 1b | **Phase D slide stitch (`_stitch_overlay_slide`)** — was the only fully serial block left | ✅ **SHIPPED (round 13)** — candidate B built, confirmed at full slide scale, and it is now the **default** (`config.stitch_backend = "tifffile"`) | **1.913x measured on Phase D** and **1.200x end-to-end** at `workers=4` on the real 27,565-tile slide — above doc 40's 1.135x projection *and* its 1.184x perfect-overlap floor. Phase D is now **20.3% of the wall**, down from 32.3%, and no longer the largest single term | Round 7 measured 322.7 s via a synthetic probe; round 8 ablated all 13 `tiffsave` knobs (all dead except `zstd`, vetoed for correctness — QuPath/BioFormats can't open it). Round 9 spiked a `tifffile` container route measuring 1.400×/2.147× **in memory**. **Round 10 re-ran it sourcing real tiles from disk and it reversed**: both candidates **0.788×/0.884× — slower than shipped**, because pyvips overlaps its read with its encode for free inside `tiffsave` while the streamed candidates paid it serially. **Round 12 built the missing lever** (`_prefetch_bands`: the band read on a background thread) and the same candidates went to **1.365×/1.581×**, all four controls reproducing doc 35 inside 3%. **Round 13 ported candidate B into the shipped `_stitch_overlay_slide` behind `config.stitch_backend` and re-measured the whole slide**: Phase D **1,889.8 → 987.8 s**, end-to-end **5,854.9 → 4,877.4 s**. Correctness veto passed on all four gates — `report.csv` 356,225 vs 356,220 rows (**+0.0014%**, tighter than round 12's −0.002%), `summary.txt` inside run-to-run noise, `overlay_pyramid_audit.py` PASS at every level, and every pyramid level bit-exact against a 2×2 box shrink of the level above. The QuPath render gate doc 35 §3.4 parked was cleared by hand 2026-07-29 before any of this was adopted. Two unpredicted effects: **peak RSS 45.6 → 17.0 GB** (band streaming replaces a 27,565-file lazy join — relaxes doc 27 §6's 61–62 GB host requirement) and the artifact is **7.50 → 5.85 GB** (Predictor 2). `pyvips` is retained as fallback and as the measurement control. **Candidate C (GPU pyramid) recommended CLOSED**, not merely deferred: it is worth ~2–3% end-to-end now that B took most of the win, against the first-ever CUDA allocation in the `workers>1` parent. Also fixed along the way (round 9): a pre-existing shipped defect where every `overlay_slide.tiff` decoded its pyramid levels as noise (`predictor="none"`, both full-slide overlays regenerated, +28.0% file size at real scale). See [`41-...`](hybrid-pipeline/41-round-13-phase-d-pipelined-stitch-implementation.md), [`40-...`](hybrid-pipeline/40-round-13-phase-d-pipelined-stitch-plan.md), [`39-...`](hybrid-pipeline/39-round-12-multiprocess-scaling-ceiling-implementation.md) §4, [`35-...`](hybrid-pipeline/35-round-10-backlog-implementation.md) §3, §1. |
