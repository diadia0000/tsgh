...
> every time. Compiled 2026-07-22, last updated 2026-07-25 for round 6 (§1 items 1 and 7 —
revised `workers` recommendation and corrected full-slide tile count/tissue fraction), by
...
| 7 | **Full real-WSI-scale validation** | **Never done, any round — and now the binding constraint on shipping item 1** | n/a | Every round (1–6) measures only 25/121/441-tile crops of a real WSI, never a complete slide end-to-end. `09-measurement-analysis-plan.md` §3.6 explicitly asked for this ("需要規劃一次完整 WSI 的跑批") and it still has not happened. Round 6 corrected two of this item's own inputs while sizing why it stayed out of scope: the real full slide is **27,565 tiles, not 35,700** (141,818×114,366 at the current tile/stride), and it is **~61% tissue-bearing**, not "mostly white background" as earlier rounds assumed. All full-WSI hour estimates on the corrected grid (now **~5.3h at `workers=1`, ~2.2h at `workers=4`** — round 6; was ~10.5h / ~3.3h at `workers=3` on the old 35,700-tile assumption) are linear extrapolations from tissue-dense crops, explicitly flagged as upper bounds. **Raised in priority by round 5, inputs corrected in round 6**: worker count interacts with tissue density (untested at scale) and with peak VRAM/RSS, which scale with N. |
| 7b | **`workers≥6` allocator balloon: one worker takes exactly 24.76 GiB and OOM-kills the batch** | **NEW (round 6), never investigated** | reliability, not speed | 2 failures in 6 `workers=6` runs (round 5b saw 0/6). Same byte-identical 24.76 GiB balloon both times, different victim tiles — the victim looks like fragmentation, the balloon does not. Under fail-fast it voids a whole batch, so it sets the safe worker ceiling; first thing to try is `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`. See [`23-...`](hybrid-pipeline/23-next-optimization-cycle-implementation.md) §4.6/§6. |
