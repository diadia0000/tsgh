...
# ============================================================
# 注意：故意不 COPY 專案原始碼進 image。
# docker-compose.yml 用 `.:/app` bind mount，容器啟動時會整個蓋掉
# /app，COPY 進去的內容永遠不會被用到，只會讓 image 白白變大
# （曾經因此吃到 190GB+，見 git log）。若之後要脫離 compose 直接
# `docker run` 這個 image，記得在這裡補上 COPY。
# ============================================================
