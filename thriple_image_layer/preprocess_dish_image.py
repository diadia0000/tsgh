...
def preprocess_dish_image(
    input_path: Path,
    output_path: Path,
    output_level: int = 1
) -> None:
    """DISH 影像前處理 - 灰階轉換"""
    print(f"[Phase 0.1] 開始處理 DISH 影像: {input_path}")
    ...
    with Pool(n_workers) as pool:
        results = list(tqdm(pool.imap(_process_tile_worker, tasks), total=len(tasks), desc="    處理分塊"))
    ...
    slide.close()
    print(f"[Phase 0.1] 完成！輸出: {output_path}")
...
if __name__ == "__main__":
    ...
    input_file = Path("../picture/WSI/DISH.tiff")
    output_file = Path("output/DISH_Gray.tiff")
    
    preprocess_dish_image(input_file, output_file)
