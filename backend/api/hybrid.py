...
# Republish the pipeline's tqdm bars as job progress (see tqdm_progress). One
# bar per stage; `phase` is the bar's desc -- the order is documented on
# `run_batch`: 載入模型 → 分析 → 匯出報表 → 縫合影像.
...
def run_hybrid_tile(body: HybridTileIn, background_tasks: BackgroundTasks) -> JobAccepted:
    ...
    return JobAccepted(job_id=submit_job(background_tasks, tqdm_progress.reporting(_run), key=_JOB_KEY))
