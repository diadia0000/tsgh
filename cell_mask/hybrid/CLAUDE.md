...
- **M3 cell analysis**
  ...
  - `m3_elastic_matching.py` — isotropically dilate DISH nucleus instances, then greedily claim them per IHC cell (each nucleus at most one owner).
  - `m3_dot_detection.py` — per-cell red-dot (CEP17) / black-dot (HER2) detection on a local LAB patch;
    matched DISH nucleus count ≥ `dot_blue_exclude_threshold` → multi-nucleus exclusion.
