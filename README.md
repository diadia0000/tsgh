...
**Cross-tile multiprocessing (`workers=N`)**: `run_batch()` (called internally, not yet exposed as a CLI flag on `hybrid_pipeline.py`) accepts a `workers` argument that runs `N` `spawn`-ed processes, each with its own model set and CUDA context, over a shared dynamic tile queue. Measured **3.09x** at `workers=3` on the reference RTX 5090 (round 5); after the `dot_detect_n_jobs` fix landed, a worker-count re-sweep (round 6) revised the recommendation down to **`workers=4`** for unattended jobs / `workers=5` when a restart is cheap — `workers≥6` showed a **2-in-6 CUDA allocator OOM** rate on the same anchor, which voids a whole fail-fast batch. It defaults to `workers=1` (today's sequential behavior; the API path never passes it). **Round 8 (2026-07-27) ran the full-WSI-scale validation that gated production and it passed**: on the real 27,565-tile slide, `workers=1` took 3.82 h and `workers=4` took 1.73 h (**2.216x measured speedup**, correctness veto passed). `workers=4` is now recommended for production, with one caveat that is about VRAM, not speed: it peaked at **93.3% of the reference card's 32 GB (~2.2 GB headroom)** — treat 32 GB VRAM as a hard floor and keep `workers≥5` off the table until the `workers≥6` allocator-OOM defect (candidate fix tried in round 8, did not clear it) is root-caused. Full detail: [`docs/hybrid-pipeline/21-cross-tile-multiprocessing-implementation.md`](docs/hybrid-pipeline/21-cross-tile-multiprocessing-implementation.md) (round 5), [`docs/hybrid-pipeline/23-next-optimization-cycle-implementation.md`](docs/hybrid-pipeline/23-next-optimization-cycle-implementation.md) §6 (round 6 re-tune), and [`docs/hybrid-pipeline/27-remaining-work-implementation.md`](docs/hybrid-pipeline/27-remaining-work-implementation.md) §5–§6 (round 8 full-slide validation). `scripts/perf_measure.py --mp-workers N` is the current way to exercise it; `run_batch(..., checkpoint=True)` (or `--resume`) enables opt-in partial-resume so an interrupted long-running batch doesn't have to restart from tile 0.
...
hard limit permits). The minimum/recommended tables above are sized for small ROI/tile
requests via the API path; a full-slide batch job needs materially more RAM and VRAM headroom
than either implies. See [`docs/hybrid-pipeline/27-remaining-work-implementation.md`](docs/hybrid-pipeline/27-remaining-work-implementation.md) §6.4/§6.6.
...
Real, measured end-to-end numbers (RTX 5090) live in [`docs/hybrid-pipeline/measurement/bottleneck-list.md`](docs/hybrid-pipeline/measurement/bottleneck-list.md) and are kept current there rather than duplicated here. **Headline (round 8, 2026-07-27) — the first complete real-WSI run this project has done**: on the real 27,565-tile slide (16.2 gigapixels), `workers=1` took **3.82 h** and `workers=4` took **1.73 h** (2.216x measured speedup), both ~40% over the crop-based projections every earlier round quoted (2.6 h / 1.25 h) — the miss traces to three costs that were measured "solved" on small crops but regress at production scale: `gc.collect` (16.1% of wall, not ~0), tile read (17.2%, not 1.22%), and the final slide-stitch (8.6%/19.3% of wall, not 3.5%/7.3%). Peak RSS **~61 GB** and `workers=4` peak GPU **~30.4 GB of 32 GB** are new, previously-unmeasured host requirements. See [`docs/hybrid-pipeline/27-remaining-work-implementation.md`](docs/hybrid-pipeline/27-remaining-work-implementation.md) §6 for the full record and [Usage](#hybrid-pipeline-main) for the current `workers=4` production recommendation.
