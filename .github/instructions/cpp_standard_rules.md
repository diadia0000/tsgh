...
❌ Do NOT:
- Leave out required `#include`.
- Use non-standard compiler extensions.
- Write platform-dependent code unless specified.
- Return raw pointers without clear ownership.
...
1. Always provide **full code with headers**.
2. Ensure code **compiles without modification**.
3. Follow the style rules above.
4. If an assumption is required, **state it explicitly in comments**.
